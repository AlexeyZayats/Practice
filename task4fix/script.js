...
var filterHelper={
  author: function(posts,author){
    return posts.filter(function (item){
      return item.author.toLowerCase().indexOf(author.toLowerCase())>=0;
    });
  },
  ...
  hashtags: function(posts,hashtags){
    return posts.filter(function(item){
       return hashtags.every(function (tag){
          return  item.hashtags.some(function (intag){
              return (intag.toLowerCase().indexOf(tag.toLowerCase())>=0);
            })
       });
    });
  }
...
var validateHelper={
  ...
  if(posts.hashtags){
    return posts.hashtags.every(function(item){
      return !!item&&item.length>1&&item.charAt(0)==='#';
    });
  }
  else{
    return true;
...
return {
  ...
  removePhotoPost: function(id){
    var index= photoPosts.findIndex(function (item){
      return item.id==id;
    });
...
console.log('getPhotoPosts(0,10,{author:lyn })');
console.log(m.getPhotoPosts(0,10,{author:'lyn'}));
...
console.log('editPhotoPost(123,{hashtags:[#aa]})');
console.log(m.editPhotoPost(123,{hashtags:['#aa']}));
console.log('editPhotoPost(123,{hashtags:[aa]})');
console.log(m.editPhotoPost(123,{hashtags:['#']}));
console.log(m.getPhotoPost(123));
